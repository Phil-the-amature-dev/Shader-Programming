Shader "Unlit/Shader1"
{
    Properties
    {
        ...
        _TextureBreathingSpeed ("Texture Breathing Speed", Float) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float4 brightness;
                float2 uvDisplacement = v.uv + float2(v.uv.x + _Time.y * 0.25,0);
            ...
            
            {
                ...
                float slant = float(fmod(_TextureWaveSpeed * ((i.uv.x + i.uv.y) + (sin(_Time.y) * _TextureWaveSpeed)),_WaveLength)); // the higher, the more to the right

                returnColor += float4(lerp(_ColorA,_ColorB,slant * sin(_Time.y) * _TextureBreathingSpeed));
